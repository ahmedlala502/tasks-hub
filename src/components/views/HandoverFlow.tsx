...
import { RefreshCw, MapPin, AlertTriangle, Send, CheckCircle2, ChevronRight, Sparkles, Loader2, Clipboard, Globe, Users, ArrowRight, MessageSquare, History, Search, Layers, Quote, FileText, Info } from 'lucide-react';
...
import { validateHandover, checkHandoverIssues, getHandoverSuggestions, getHandoverReadiness } from '../../lib/handoverValidationService';
...
export default function HandoverFlow({ handovers, tasks, stats, aiInteractions }: HandoverFlowProps) {
  ...
  const autoSelectTasks = useCallback((useTemplate?: HandoverTemplate) => {
    if (useTemplate) {
      setNewHo(prev => {
        const team = useTemplate.team || prev.team;
        const ids = Array.from(new Set(
          tasks
            .filter(t => (t.carry || t.priority === Priority.HIGH) && t.status !== Status.DONE && (!team || t.team === team))
            .map(t => t.id)
        ));
        return {
          ...prev,
      ...
      const team = newHo.team;
      const criticalIds = Array.from(new Set(
        tasks
          .filter(t => (t.carry || t.priority === Priority.HIGH) && t.status !== Status.DONE && (!team || t.team === team))
          .map(t => t.id)
      ));
  ...
  const saveHandover = async () => {
    const validationErrors = validateHandover(newHo, tasks);
    const blockingErrors = validationErrors.filter(e => e.severity === 'error');
    if (blockingErrors.length > 0) {
      addToast(blockingErrors[0].message, 'error', 4000);
      return;
    }

    const conflicts = checkHandoverIssues(newHo, tasks, handovers);
    if (conflicts.length > 0) {
      addToast(conflicts[0], 'warning', 5000);
    }
    ...
    const fresh: Partial<Handover> = {
      ...
      taskIds: Array.from(new Set(newHo.taskIds || [])),
      createdAt: new Date().toISOString(),
      creatorId: user.email || 'local-workspace',
      quality: 'good',
    };
  ...
  const readiness = useMemo(() => getHandoverReadiness(newHo, tasks), [newHo, tasks]);
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 4 && (
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} key="s4" className="space-y-8">
              ...
              {/* Smart suggestions from validation service */}
              {(() => {
                const suggestions = getHandoverSuggestions(newHo, tasks);
                return suggestions.length > 0 ? (
                  <div className="space-y-2 p-4 bg-amber-50 border border-amber-200 rounded-2xl">
                    {suggestions.map((s, i) => (
                      <div key={i} className="flex items-start gap-2 text-[11px] font-bold text-amber-700">
