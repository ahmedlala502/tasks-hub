...
export default function DailyRoutines() {
  ...
  const assignmentOptions = useMemo(() => buildAssignmentOptions({ users: TEAM_MEMBERS, tasks, campaigns, handovers: dataService.getHandovers() }), [campaigns, tasks]);
        ...
        <div className="grid grid-cols-1 gap-5 xl:grid-cols-[0.82fr_1.18fr]">
          ...
          <section className="rounded-xl border border-border bg-card p-5">
            <div className="grid gap-4 xl:grid-cols-3">
              <TaskLane title="Focus Queue" tasks={focus.queue} onEdit={(task) => setDraft(draftFromTask(task))} onStatus={updateStatus} onCreate={() => openCreate('PMO')} />
              {PMO_LANES.slice(0, 2).map((lane) => <TaskLane key={lane} title={lane} tasks={visibleTasks.filter((task) => (task.department || task.category || 'Operations') === lane).slice(0, 10)} onEdit={(task) => setDraft(draftFromTask(task))} onStatus={updateStatus} onCreate={() => openCreate(lane)} />)}
            </div>
            <div className="mt-4 grid gap-4 xl:grid-cols-4">
              {PMO_LANES.slice(2).map((lane) => <TaskLane key={lane} title={lane} tasks={visibleTasks.filter((task) => (task.department || task.category || 'Operations') === lane).slice(0, 8)} onEdit={(task) => setDraft(draftFromTask(task))} onStatus={updateStatus} onCreate={() => openCreate(lane)} compact />)}
            </div>
          </section>
        </div>
      ) : (
                ...
                {visibleTasks.map((task) => (
                  <tr key={task.id}>
                    ...
                    <td className="px-4 py-3">
                      <select className="settings-input h-8 text-[11px]" value={getOperationalTaskStatus(task)} onChange={(event) => updateStatus(task, event.target.value as NonNullable<Task['status']>)}>
                        {['Pending', 'In Progress', 'Blocked', 'Done'].map((status) => <option key={status} value={status}>{status}</option>)}
                      </select>
                    </td>
                    <td className="px-4 py-3">
                      <button onClick={() => setDraft(draftFromTask(task))} className="icon-btn" title="Edit task"><Edit3 size={14} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
...
function TaskLane({ title, tasks, onEdit, onStatus, onCreate, compact = false }: { title: string; tasks: Task[]; onEdit: (task: Task) => void; onStatus: (task: Task, status: NonNullable<Task['status']>) => void; onCreate: () => void; compact?: boolean }) {
  return (
        ...
        {tasks.length ? tasks.map((task) => (
          <div key={task.id} className="rounded-lg border border-border bg-card p-3">
            <div className="flex items-start justify-between gap-2">
              ...
              <button onClick={() => onEdit(task)} className="icon-btn" title="Edit task"><Edit3 size={14} /></button>
            </div>
            <select className="settings-input mt-2 h-8 text-[11px]" value={getOperationalTaskStatus(task)} onChange={(event) => onStatus(task, event.target.value as NonNullable<Task['status']>)}>
              {['Pending', 'In Progress', 'Blocked', 'Done'].map((status) => <option key={status} value={status}>{status}</option>)}
            </select>
          </div>
